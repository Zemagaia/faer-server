...
namespace terrain
{
    unsafe class Rasterizer<T>
    {
        ...
        public void DrawPolygon(double[] points, T val)
        {
            ...
            var pixels = Buffer;
            int pn = points.Length;
            double prevX = points[pn - 2];
            double prevY = points[pn - 1];

            // Draw line between each vertex and the previous vertex
        ...
        public void DrawRectangle(int x, int y, int width, int height, T val, int lineWidth)
        {
            int xEnd = x + width - 1;
            int yEnd = y + height - 1;

            DrawLineBresenham(x, y, xEnd, y, val, lineWidth);
            DrawLineBresenham(x, y, x, yEnd, val, lineWidth);
            DrawLineBresenham(xEnd, y, xEnd, yEnd, val, lineWidth);
            DrawLineBresenham(x, yEnd, xEnd, yEnd, val, lineWidth);
        }

        public void DrawCircle(double centerX, double centerY, double radius, T val, int thickness)
        {
            for (int i = 0; i < thickness; i++)
            {
                ...
                while (y <= x)
                {
                    Plot((int)(centerX + x), (int)(centerY + y), val);
                    Plot((int)(centerX + y), (int)(centerY + x), val);
                    Plot((int)(centerX - x), (int)(centerY + y), val);
                    Plot((int)(centerX - y), (int)(centerY + x), val);
                    Plot((int)(centerX - x), (int)(centerY - y), val);
                    Plot((int)(centerX - y), (int)(centerY - x), val);
