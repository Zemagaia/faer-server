...
namespace GameServer.realm
{
    ...
    public struct MapTile
    {
        ...
        public ObjectDef ToDef(int x, int y)
        {
            ...
            {
                ObjectType = (ushort) Desc.ObjType,
    ...
    public class JsonMap
    {
        ...
        public IEnumerable<Entity> InstantiateEntities()
        {
            ...
            {
                var entity = Entity.Resolve(null, XmlData.id2type_obj[i.Item2.id]);
        ...
        MapTileDesc loc2Tile(loc loc, List<obj> objs)
        {
            ...
            if (loc.ground != null)
                ret.Tile = (Tile)XmlData.id2type_tile[loc.ground];
            else
                ret.Tile = (Tile)0xff;
            ...
            if (loc.objs != null)
            {
                ...
                {
                    ObjectDesc desc;
                    if (XmlData.objDescs.TryGetValue(XmlData.id2type_obj[z.id], out desc))
                    {
                        if (desc.Static && !desc.Enemy)
                        {
                            ret.ObjType = (short) desc.ObjectType;
                            ret.Name = z.name;
                        }
                        else
                            objs.Add(z);
