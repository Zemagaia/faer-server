...
public class Client {
	...
	private async void Receive()
	{
		...
		try
		{
			...
			{
				int num;
				var len = (num = await Socket.ReceiveAsync(ReceiveMem));
				if (num > 0)
				{
					ProcessPacket(len);
				}
	...
	private async void TrySend(int len)
	{
		...
		catch (Exception e)
		{
			...
			{
				var log = Log;
				var defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(28, 3);
	...
	public void SendAccountList(int id, int[] list)
	{
		...
		WriteByte(ref ptr, ref spanRef, 42);
	...
	public void SendAllyShoot(byte bulletId, int ownerId, ushort containerType, float angle)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 50);
	...
	public void SendAOE(float x, float y, float radius, ushort damage, ConditionEffectIndex effect, float duration, ushort origType, uint color)
	{
		...
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 32);
	...
	public void SendBuyResult(int id, string res)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 31);
		WriteInt(ref ptr, ref spanRef, id);
	...
	public void SendCreateSuccess(int objId, int charId)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 1);
	...
	public void SendDamage(int targetId, ConditionEffects effects, ushort damage, bool kill, byte bulletId, int objectId)
	{
		...
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 8);
	...
	public void SendDeath(int accId, int charId, string killedBy)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 29);
	...
	public void SendEnemyShoot(byte bulletId, int ownerId, byte bulletType, float x, float y, float angle, short damage, byte numShots, float angleInc)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 51);
		WriteByte(ref ptr, ref spanRef, bulletId);
		WriteInt(ref ptr, ref spanRef, ownerId);
	...
	public void SendGoto(int objId, float x, float y)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 17);
		WriteInt(ref ptr, ref spanRef, objId);
		WriteFloat(ref ptr, ref spanRef, x);
		WriteFloat(ref ptr, ref spanRef, y);
		TrySend(ptr);
	...
	public void SendGuildResult(bool success, string errorText)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 47);
	...
	public void SendInvitedToGuild(string guildName, string name)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 65);
	...
	public void SendInvResult(int result)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 19);
	...
	public void SendNameResult(bool success, string errorText)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 45);
		WriteBool(ref ptr, ref spanRef, success);
		WriteString(ref ptr, ref spanRef, errorText);
		TrySend(ptr);
	}
	...
	public void SendMapInfo(int width, int height, string name, string displayName, int diff, int background, bool allowTp, bool showDisplays)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 23);
	...
	public void SendNewTick(int tickId, int tickTime, ObjectStats[] stats)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 12);
		...
		for (var i = 0; i < stats.Length; i++)
		{
			...
			foreach (var keyValuePair in statTypes)
			{
				keyValuePair.Deconstruct(out var key2, out var value2);
				SendStat(ref ptr, ref spanRef, key2, value2);
			}
		}
		TrySend(ptr);
	}
	...
	public void SendNotification(int objId, string msg, uint color)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 11);
		WriteInt(ref ptr, ref spanRef, objId);
	...
	public void SendPing(int serial)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 21);
	...
	public void SendShowEffect(EffectType effectType, int targetObjId, float x1, float y1, float x2, float y2, uint color)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 15);
	...
	public void SendText(string name, int objectId, byte bubbleTime, string recipient, string text, uint nameColor = 0xFF0000, uint textColor = 0xFFFFFF)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 6);
		WriteString(ref ptr, ref spanRef, name);
		WriteInt(ref ptr, ref spanRef, objectId);
	...
	public void SendTradeAccepted(bool[] myOffer, bool[] yourOffer)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 60);
	...
	public void SendTradeDone(int code, string desc)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 59);
	...
	public void SendTradeRequested(string name)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 53);
		WriteString(ref ptr, ref spanRef, name);
		TrySend(ptr);
	}
	...
	public void SendTradeStart(TradeItem[] myItems, string yourName, TradeItem[] yourItems)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 54);
	...
	public void SendUpdate(TileData[] tiles, ObjectDef[] newObjs, int[] drops)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 9);
	...
	public void SendFailure(string text, FailureType errorId = FailureType.MessageWithDisconnect)
	{
		var ptr = 0;
		ref var spanRef = ref MemoryMarshal.GetReference(SendMem.Span);
		WriteByte(ref ptr, ref spanRef, 72);
	...
	{
		if (a == null || b == null || !(a is IContainer) || !(b is IContainer) || (a is Player && a != p) || (b is Player && b != p) || (a is Container container && container.BagOwners.Length != 0 && !container.BagOwners.Contains(p.AccountId)) || (b is Container container2 && container2.BagOwners.Length != 0 && !container2.BagOwners.Contains(p.AccountId)))
		{
			return false;
		}
	...
	{
		return c == player || item == null || (!item.Untradable && !player.Client.Account.Admin) || IsSoleContainerOwner(player, c as IContainer);
	}
	...
	private void ProcessPacket(int len) {
		...
		while (ptr < len) {
			...
			case PacketId.AcceptTrade:
				ProcessAcceptTrade(ReadBoolArray(ref ptr, ref spanRef, len), ReadBoolArray(ref ptr, ref spanRef, len));
				break;
			case PacketId.AoeAck:
				ProcessAoeAck(ReadInt(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len));
				break;
			case PacketId.Buy:
			...
			case PacketId.EditAccountList:
				ProcessEditAccountList(ReadInt(ref ptr, ref spanRef, len), ReadBool(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len));
				break;
			...
			case PacketId.GroundDamage:
				ProcessGroundDamage(ReadInt(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len));
				break;
			...
			case PacketId.InvDrop:
				ProcessInvDrop(ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len), ReadShort(ref ptr, ref spanRef, len));
				break;
			case PacketId.InvSwap:
				ProcessInvSwap(ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len));
				break;
			case PacketId.JoinGuild:
				ProcessJoinGuild(ReadString(ref ptr, ref spanRef, len));
				break;
			case PacketId.Move:
				ProcessMove(ReadInt(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadTimedPosArray(ref ptr, ref spanRef, len));
				break;
			case PacketId.OtherHit:
				ProcessOtherHit(ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len));
				break;
			...
			case PacketId.PlayerShoot:
				ProcessPlayerShoot(ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len), ReadUShort(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len));
				break;
			...
			case PacketId.UseItem:
				ProcessUseItem(ReadInt(ref ptr, ref spanRef, len), ReadInt(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len), (ushort)ReadShort(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadFloat(ref ptr, ref spanRef, len), ReadByte(ref ptr, ref spanRef, len));
				break;
	...
	private void ProcessChangeGuildRank(string name, int rank)
	{
		...
		var targetCli = Manager.Clients.Keys.SingleOrDefault((Client c) => c.Account.get_AccountId() == targetId);
		var targetAcnt = ((targetCli != null) ? targetCli.Account : Manager.Database.GetAccount(targetId, (string)null));
		...
		if (targetRank < rank)
		{
			Manager.Chat.Guild(Player, targetAcnt.get_Name() + " has been promoted to " + text + ".", announce: true);
		}
		else
		{
			Manager.Chat.Guild(Player, targetAcnt.get_Name() + " has been demoted to " + text + ".", announce: true);
		}
	}
	...
	private void ProcessCreateGuild(string guildName)
	{
		...
		var guildRes = Manager.Database.CreateGuild(guildName, ref createdGuild);
		if ((int)guildRes > 0)
		{
	...
	private void ProcessHello(string buildVer, int gameId, string guid, string pwd, int charId, bool createChar, ushort charType, ushort skinType)
	{
		...
		if (createChar)
		{
			var character = default(DbChar);
			var status = Manager.Database.CreateCharacter(acc, charType, skinType, ref character);
			switch ((int)status)
			{
			case 1:
